...
- **`warden`** (character): 48x48 bitmap regions with a five-step steel ramp, gold
  trim and a lit visor. The current high-water mark for art quality in this repo.
...
```
built 8 asset(s), 323 finding(s) total
  beacon: ok
  crawler: ok
  engineer: ok
  forest_tileset: ok
